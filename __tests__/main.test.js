  ...
  describe("Basic functionality", () => {
    ...
    it("should handle already existing link gracefully", async () => {
      ...
      const scope = nock("https://dev.azure.com")
        .post(
          "/test-org/_apis/Contribution/dataProviders/query?api-version=7.1-preview.1"
        )
        .reply(200, {
          data: {
      ...
      mockWorkItemTrackingApi.updateWorkItem.mockRejectedValue(
        new Error("The relation already exists")
      );
    ...
    it("should send correct data provider request structure", async () => {
      ...
      expect(requestBody.context.properties.urls[0]).toBe(
        "https://github.com/owner/repo/pull/42"
      );
      expect(requestBody.contributionIds[0]).toBe(
        "ms.vss-work-web.github-link-data-provider"
      );
